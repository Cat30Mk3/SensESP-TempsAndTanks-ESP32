// February 24 2025 -- fully functioning including averaging tach to NMEA2000

// todo .. seems to be a bug in enable temps and tach -- always appears enabled but isnt
//      .. extract version from __filename__
//      .. confirm tach enable and n2k code is inplace and working
//      .. display update seems to stall during rotary encoder twist

//future:
// see it can take advantage of builtin vector manipulation tools like movingavaerage.cpp does
// see if can figure out how to add variables to config ui
// see if can structure code more like is used in version 3 examples
...
double tachAverager(float multiplier_, double input){

  if (!averagingIntialized) {
    for(int i=0;i<TACH_AVG_MAX_COUNT;i++)averagingArray[i]=input;
    averagingOutput = input;
    averagingIntialized = true;
  }
  else{
    // Subtract 1/nth of the oldest value and add 1/nth of the newest value
    averagingOutput += -multiplier_ * averagingArray[averagingIndex] / TACH_AVG_MAX_COUNT;
    averagingOutput += multiplier_ * input / TACH_AVG_MAX_COUNT;
    ...
    averagingArray[averagingIndex] = input;
    averagingIndex = (averagingIndex + 1) % TACH_AVG_MAX_COUNT;
  }
  return averagingOutput;
}
...
//    if (testRawRpm < ENGINE_MAX_RPM) tachParam[0].tachReadingRpm = tachAverager(1.0,testRawRpm);
...
void SendEngineSpeed(int index) {
  ...
  SetN2kPGN127488(N2kMsg,
                  /*EngineInstance*/ 0,
                  ///*EngineSpeed*/ 1527,
                  /*EngineSpeed*/ tachParam[index].tachAverageRpm,
...
// #endif
  ...
  pinMode(HALL_EFFECT_SENSOR_PIN, INPUT_PULLUP);                  //OLD TACH
  attachInterrupt(HALL_EFFECT_SENSOR_PIN, tachIsr, RISING);       //OLD TACH
  ...
  if (remoteNotLocal) {
    if (CheckI2C(i2c, DISPLAY_REMOTE_I2C_ADDRESS)) {
      ...
      //rotaryEncoder.setPins(REMOTE_ENC_A_VIOLET, REMOTE_ENC_B_GREY,REMOTE_PB_ENCODER);
      selectedEncoder = REMOTE_ENCODER;
    } else {
      startupDisplayAddress = DISPLAY_LOCAL_I2C_ADDRESS;
      //rotaryEncoderSelected[selectedEncoder].setPins(LOCAL_ENC_A_VIOLET, LOCAL_ENC_B_GREY,
      //                      LOCAL_PB_ENCODER);
      selectedEncoder = LOCAL_ENCODER;
    };
  } else {
    if (CheckI2C(i2c, DISPLAY_LOCAL_I2C_ADDRESS)) {
      startupDisplayAddress = DISPLAY_LOCAL_I2C_ADDRESS;
      //rotaryEncoderSelected[selectedEncoder].setPins(LOCAL_ENC_A_VIOLET, LOCAL_ENC_B_GREY, LOCAL_PB_ENCODER);
      selectedEncoder = LOCAL_ENCODER;
    } else {
      startupDisplayAddress = DISPLAY_REMOTE_I2C_ADDRESS;
      //rotaryEncoderSelected[selectedEncoder].setPins(REMOTE_ENC_A_VIOLET, REMOTE_ENC_B_GREY, REMOTE_PB_ENCODER);
      selectedEncoder = REMOTE_ENCODER;
    };
  }
  DEBUG_PRINTF("\n** selected Encoder is: %d **\n\n",selectedEncoder);
  rotaryEncoderSelected[selectedEncoder].setEncoderType(EncoderType::FLOATING);  //FLOATING causes internl pull-ups to be used
  rotaryEncoderSelected[selectedEncoder].setBoundaries(-1, 1, false);
  rotaryEncoderSelected[selectedEncoder].onTurned(&knobCallback);
  rotaryEncoderSelected[selectedEncoder].onPressed(&buttonCallback);
  rotaryEncoderSelected[selectedEncoder].begin();
  ...
  ConfigItem(main_engine_oil_temperature)
    ->set_title("Engine Oil Temperature")
    ->set_description("Engine Oil Temperature")
    ->set_sort_order(100);
  ...
  ConfigItem(main_engine_coolant_temperature)
    ->set_title("Engine Coolant Temperature")
  ...
  ConfigItem(main_engine_exhaust_temperature)
      ->set_title("Engine Exhaust Temperature")
      ->set_description("Engine Exhaust Temperature")
      ->set_sort_order(100);
  ...
  // ConfigItem(tank_0_volume)
  //   ->set_title("Tank 0 Volume")
  //   ->set_description("Tank 0 Volume")
  //   ->set_sort_order(100);
...
//auto tacho_d1_frequency = ConnectTachSender(kDigitalInputPin1, "main");
//auto tach_0_rpm = ConnectTachoSender(HALL_EFFECT_SENSOR_PIN,"tach_0_rpm");   //TACH TRIAL
...
//**** Tach connection to NMEA 200 is missing !! ****
  // tach_0_rpm->connect_to(new LambdaConsumer<float>([](float value) {   //TACH TRIAL
 ...
 // app.onRepeat(1, []() { nmea2000->ParseMessages(); });   //DEPRECATED
 // YES! .. this is needed even though not looking for incoming NMEA messages
  event_loop()->onRepeat(1, []() { nmea2000->ParseMessages(); });

  //app.onRepeatMicros(10000, []() { handle_events(); });   //DEPRECATED
  event_loop()->onRepeatMicros(10000, []() { handle_events(); }); 

  // app.onRepeatMicros(1000000, []() {  //changed from 500,000  //DEPRECATED
  //   checkEngineStopped();
  //   newLiveDisplayBuilder();
  // });

  event_loop()->onRepeat(1000, [](){  //changed from 500,000
    checkEngineStopped();
    newLiveDisplayBuilder();
    tachParam[0].tachAverageRpm = tachAverager(1.0,tachParam[0].tachReadingRpm);
    SendEngineSpeed(0);
  });



  // app.onRepeat(1000, []() {    //DEPRECATED
  //   checkEngineStopped();
  ...
  //sensesp_app->start();  //DEPRECATED
}
// main program loop

//void loop(void) { app.tick(); }  //DEPRECATED
